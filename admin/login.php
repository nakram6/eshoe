...
File Name: login.php
Description: This file is login page of admin
Dated:  08/08/2022
...
if(isset($_POST['login_btn'])){
  ...
  $stmt = $conn->prepare("SELECT admin_id,name, email, password FROM admins WHERE email = ? AND password = ? LIMIT 1");

  $stmt->bind_param('ss',$email,$password);

  if($stmt->execute()){
      $stmt->bind_result($admin_id,$name,$email,$password);
      $stmt->store_result();
